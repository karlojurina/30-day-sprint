...
| v76 | Jun 12 | **Canceling feature pair** (Karlo-requested): (1) `is_canceling` condition in the template builder — Karlo creates a save-the-sale DM template in /admin/templates that fires within 4h of a student entering Canceling and auto-dismisses on churn/reactivation; (2) real trend line on the dashboard Canceling tile via the new `canceling_snapshots` table (v84 migration — separate table because the rebuild RPC deletes+reinserts `daily_progress_snapshots`, which would wipe unrecomputable point-in-time state). History accrues from ship date |
| v75.61 | Jun 11 | churn last-resort stamp: a stored-terminal student with NULL `canceled_at` and NO usable past end-date from Whop now gets `canceled_at = now()` (observation moment) instead of staying NULL forever. Spec (Karlo): churned = lost access (period-end lapse OR refund revocation) and must NEVER be invisible to the churn trend. Whop sends only the churn STATE, never the transition timestamp — the date is always ours to record |
| v75.60 | Jun 11 | refund-churn auto-restamp: `whopEndIso` now falls back to `expires_at` (access-revocation moment) when `renewal_period_end` is future-dated. Refunded students churn mid-cycle, so the old `renewal_period_end ?? expires_at` never consulted expires_at and a lost `canceled_at` could never be auto-repaired — the 2 refund churns invisible on the dashboard tile while the journey (status-based) showed them. Whop exposes NO cancellation timestamp (v75.16.4 note), so access-end is the best automatic proxy |
