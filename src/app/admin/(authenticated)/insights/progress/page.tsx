...
// v75.50: scope toggle removed — every metric on this page is now
// LAUNCH COHORT only. No more useAdminScope branching.
...
// v75.50: 'joined_count' removed — Karlo's v75.43 directive
// ('I don't think Joined is relevant for us, because we see that in
// the Whop dashboard') applies to insights too.
type MetricKey =
  | "avg_progress"
  | "active_count"
  | "churned_count";
...
const METRICS: Record<MetricKey, MetricDef> = {
  ...
  active_count: {
    ...
    description:
      "Launch-cohort paying members with active or past-due status.",
    suffix: "",
    color: "#5bb88e",
    mode: "running",
  },
...
export default function ProgressInsightsPage() {
  ...
  // v75.50: removed useAdminScope. Page is cohort-only.
  const paceCounts = useJourneyPaceCounts();
  const bounty = useBountyInsights();
  const sprinters = useCurrentSprinterProgress();

  // v75.50: replaced the v75.32-era /api/admin/insights/progress
  // fetch with a direct supabase query. No need for a separate API
  // route — daily_progress_snapshots has admin-only RLS and the
  // client supabase session already authenticates as the team
  // member. Simpler architecture, one fewer file to maintain.
    ...
    try {
      const fromIso =
        preset === "custom"
          ...
          : preset === "all"
            ? "2026-01-01"
            : daysAgoIso(parseInt(preset, 10));
      const toIso = preset === "custom" ? customTo : todayIso();
      const { data, error: queryErr } = await supabase
        .from("daily_progress_snapshots")
        .select(
          "snapshot_date, avg_progress, active_students, total_completions, " +
            "active_count, joined_count, churned_count, " +
            "avg_progress_cohort, active_count_cohort, joined_count_cohort, churned_count_cohort",
        )
  ...
  // v75.50: always read the *_cohort columns. Falls back to the
  // legacy non-cohort column for pre-v77 snapshot rows that don't
  // have the cohort family yet — so historical charts still render.
...
function useBountyInsights(): BountyInsights {
  ...
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      // v75.50: paginated + cohort-filtered. Previously this fetched
      // every student_milestones row unpaginated (PostgREST silently
      // truncated past 1000), AND it had no cohort/paying-plan
      // filter, so it leaked legacy + v50-backfilled + free-plan
      // stamps into the count. Now: fetch launch-cohort paying
      // student ids first, then fetch milestones paginated, then
      // intersect. Same shape as dashboard's v75.44 + v75.48 fix.
      const [studentsRes, milestonesRes] = await Promise.all([
        fetchAllRowsPaginated<{ id: string }>(() =>
          supabase
            .from("students")
            .select("id")
      ...
      const claimedAts = (milestonesRes.data ?? [])
        .filter((m) => cohortIds.has(m.student_id))
        .map((m) => m.bounty_access_claimed_at as string)
        .filter(Boolean);
