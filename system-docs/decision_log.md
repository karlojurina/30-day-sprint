...
| 2026-08-28 | Cap the loop instead of hunting its trigger: module-level call gates + client telemetry (v85.14) | Five fixes chased the TRIGGER of a ~1/sec request loop (800 queries in 72s from one tab) and none was confirmed. A cap holds whether or not the diagnosis is right, and nothing legitimate needs a profile 70x/min. `refreshFromServer` throttles by default; user-clicked callers opt out. Blocked calls POST to `/api/client-event` \u2014 first client-side visibility this app has had |
